...
public class SchoolworkAssist implements Parcelable {
    ...
    public SchoolworkAssist(String username, String password, Context ctx) {
    ...
    public class JsInteration {
        ...
        OnFetched listener = null;

        JsInteration(OnFetched onFetched) {
            listener = onFetched;
        }

        @JavascriptInterface
        public void setResult(String res) throws LoginException, IOException {
            listener.onFetched(res);
        }
    }
    ...
    private void fetchLoginParams(OnFetched onFetched) throws IOException {
        ...
        final Connection.Response[] res = {Jsoup.connect(LOGIN_URL).header(USER_AGENT_HEADER, USER_AGENT)
        ...
        String text = res[0].body();

        Document doc = Jsoup.parse(text);

        Elements ltEl = doc.getElementsByAttributeValue("name", "lt");
        if (ltEl.size() > 0) {
            lt = ltEl.first().attr("value").trim();
        }

        Elements exeEl = doc.getElementsByAttributeValue("name", "execution");
        if (exeEl.size() > 0) {
        ...
        new Handler(Looper.getMainLooper()).post(new Runnable() {
            ...
            public void run() {
                ...
                myWebView.addJavascriptInterface(new JsInteration(onFetched), "control");
                ...
                myWebView.setWebViewClient(new WebViewClient() {
                    ...
                    public void onPageFinished(WebView view, String url) {
                        ...
                        //myWebView.loadUrl("javascript:hi()");
                      myWebView.loadUrl("javascript:run(\"" + getUsername() + "\",\""
                       + getPassword() + "\",\"" + lt + "\")");
                    }

                });
                myWebView.loadUrl("file:///android_asset/rsa.htm");
            }
        });
    ...
    public void login() throws IOException, LoginException {
        ...
        fetchLoginParams(rsa -> {
            ...
            Document doc = Jsoup.connect(LOGIN_URL)
                    ...
                    .header(USER_AGENT_HEADER, USER_AGENT)
                    .header(HEADER_CONTENT_TYPE, CONTENT_TYPE)
                    .header(HEADER_REFERER, "http://cas.bnu.edu.cn/cas/login?service=http%3A%2F%2Fzyfw.bnu.edu.cn%2F")
//                .data("username", getUsername())
//                .data("password", getPassword())
                    .data("ul", String.valueOf(getUsername().length()))
                    .data("pl", String.valueOf(getPassword().length()))
                    .data("rsa", rsa)
                    .data("code", "code")
                    .data("lt", lt)
    ...
    public ArrayList<ExamScore> getExamScores(int year, int term, boolean major) throws IOException, NeedLoginException {
        ...
        for (Element tr : rows) {
            ...
            examScore.setFirstLearn("初修".equals(columns.get(5).text().trim()));
            examScore.setUsualScore(columns.get(6).text());
            examScore.setExamScore(columns.get(7).text());
            examScore.setScore(columns.get(8).text());
            examScore.setMajor("主修".equals(columns.get(9).text().trim()));

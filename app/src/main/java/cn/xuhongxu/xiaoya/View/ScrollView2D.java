...
public class ScrollView2D extends FrameLayout {
    ...
    protected float getBottomFadingEdgeStrength() {
        ...
        final int span = (int) (getChildAt(0).getBottom()/mScale) - getScrollY() - bottomEdge;
        if (span < length) {
    ...
    protected float getRightFadingEdgeStrength() {
        ...
        final int span = (int) (getChildAt(0).getRight()/mScale) - getScrollX() - rightEdge;
        if (span < length) {
            return span / (float) length;
        }
        return 1.0f;
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            ...
            if (getChildCount() > 0) {
                View child = getChildAt(0);
                scrollTo(clamp(x, getWidth() - getPaddingRight() - getPaddingLeft(), (int) (child.getWidth() / mScale)),
                        clamp(y, getHeight() + getPaddingBottom() - getPaddingTop(), (int) ((child.getHeight() + child.getPaddingBottom()) / mScale)));
            } else {
                scrollTo(x, y);
            }
